    ...
    if (jids.length === 0 && ctx.args && ctx.args.length > 0) {
        for (const arg of ctx.args) {
            ...
            if (normalized.includes('@')) {
                ...
                try {
                    if (client.socket) {
                        const resolved = await resolveUsername(client.socket, cleanArg, client.usernameCache);
                        if (resolved) {
                            jids.push(resolved);
                        }
                    }
