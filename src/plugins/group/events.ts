...
export const groupEventsPlugin: Plugin = {
  name: 'group-events',
  setup(client: Client) {
    // User join group
    client.on('group-join', async (payload) => {
      ...
      for (const participant of participants) {
        ...
        await client.send(groupId).text(welcomeMessage).mentions([jid]);
      }
    });

    // User leave group
    client.on('group-leave', async (payload) => {
      const { groupId, participants } = payload;
      ...
      for (const participant of participants) {
        ...
        await client.send(groupId).text(goodbyeMessage).mentions([jid]);
      }
    });
  }
};
